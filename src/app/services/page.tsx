  ...
  const roadServices = [
    {
      ...
      description:
        "Nous gérons les démarches administratives pour le changement de propriétaire.",
      icon: <FileText className="w-10 h-10 text-brand-blue" />,
    },
    {
      title: "Carte Grise Perdue",
      description:
        "Assistance pour la demande de duplicata en cas de perte ou de vol.",
        ...
        <div className="mb-20">
          <h2 className="text-3xl font-bold text-secondary mb-10 text-center relative inline-block w-full">
            <span className="relative z-10 bg-white px-4">
              Services Routiers & Administratifs
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {roadServices.map((service, index) => (
              <div
                key={index}
                className="bg-gray-50 rounded-xl p-8 hover:shadow-md transition border border-gray-100 group"
              >
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-gl mx-auto">
            {otherServices.map((service, index) => (
              <div
                key={index}
                className="bg-gray-50 rounded-xl p-8 hover:shadow-md transition border border-gray-100 flex items-start"
              >
                <div className="mr-6 shrink-0">{service.icon}</div>
                <div>
